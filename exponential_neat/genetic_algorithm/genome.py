...
class Genome:
    ...
    def newNet(self, random_weights: bool = True) -> nx.DiGraph:
        ...
        w_edges = [(i, out_node_id, {"weight": weight_gen(), "gin": i}) for i in range(self.num_features)]
        net.add_edges_from(w_edges)
        nx.set_node_attributes(net, {out_node_id: True}, name=OUTPUT_NODE_NAME) 
        for i in range(self.num_features):
            nx.set_node_attributes(net, {i: i for i in range(self.num_features)}, name="feature")
        return net
    ...
    @classmethod
    def newNode(cls, net: nx.DiGraph, edge2split: Tuple[int, int] | None = None) -> nx.DiGraph:
        net = deepcopy(net)
    ...
    def newConnection(cls, net: nx.DiGraph) -> nx.DiGraph:
        ...
        out_node = [node for node in net.nodes(data=True) if OUTPUT_NODE_NAME in node[1]][0]
        ...
        for _ in range(1000):
            ...
            if nx.is_directed_acyclic_graph(nc):
                nx.set_edge_attributes(nc, {(n1, n2): {"gin": next(cls.global_innovation_number)}})
                return nc
    ...
    @classmethod
    def __evaluate_helper(cls, net: nx.DiGraph, out_node: Tuple[int, dict], dataset: np.array) -> np.array:
